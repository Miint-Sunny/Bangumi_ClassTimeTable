...
import { DAY_MS, WEEKDAY_CN, dayOrder, displayTz, occurrenceOn, partsInZone, slotFor } from '../lib/time'
    ...
    const push = (cy: number, cmo: number, cd: number, out: boolean) => {
      ...
      if (!out) {
        for (const s of shows) {
          const ep = occurrenceOn(s, cy, cmo, cd, tz)
          if (ep === null) continue
          const occ = s.begin! + (ep - 1) * s.periodDays * DAY_MS
          entries.push({ show: s, ep, minutes: slotFor(s, settings).minutes, aired: occ <= now })
        }
        entries.sort((a, b) => a.minutes - b.minutes || a.show.id - b.show.id)
      }
      cells.push({
        y: cy,
        mo: cmo,
                ...
                {c.entries.map((e) => {
                  ...
                  return (
                    <button
                      ...
                      title={`${e.show.nameCn} 第${e.ep}集`}
                      onClick={() => onOpen(e.show.id)}
                    >
                      <span className="ep">{String(e.ep).padStart(2, '0')}</span>
