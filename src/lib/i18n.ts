...
const DICT: Record<string, [string, string, string]> = {
  ...
  动作: ['動作', 'アクション', 'Actions'],
  '{n} 个活跃日': ['{n} 個活躍日', '{n} 日活動', '{n} active days'],
  深夜党指数: ['深夜黨指數', '深夜派指数', 'Night-owl index'],
  '0–5 点占比': ['0–5 點佔比', '0〜5時の割合', 'share at 0–5h'],
  一集平均耗时: ['一集平均耗時', '1話あたりの間隔', 'Per-episode pace'],
  同一部相邻两话的中位间隔: ['同一部相鄰兩話的中位間隔', '同作品の連続2話の中央値間隔', 'median gap between consecutive eps of one show'],
  '{n} 小时': ['{n} 小時', '{n} 時間', '{n} h'],
  '{n} 天': ['{n} 天', '{n} 日', '{n} d'],
  连续追番周数: ['連續追番週數', '連続視聴週数', 'Week streak'],
  最长追番: ['最長追番', '最長追跡', 'Longest follow'],
  最忙的一天: ['最忙的一天', '最も忙しい日', 'Busiest day'],
  '{m}月': ['{m}月', '{m}月', 'M{m}'],
  '{n} 次': ['{n} 次', '{n} 回', '{n}'],
  少: ['少', '少', 'Less'],
  多: ['多', '多', 'More'],
  ...
  周六: ['週六', '土', 'Sat'],
  周日: ['週日', '日', 'Sun'],
  口味逆风盘: ['口味逆風盤', '逆張り度', 'Taste vs crowd'],
  挑剔度: ['挑剔度', '辛口度', 'Pickiness'],
  我的分减站均分的平均: ['我的分減站均分的平均', '自分の点−サイト平均の平均', 'mean of my score − site score'],
  与站均的相关系数: ['與站均的相關係數', 'サイト平均との相関', 'Correlation with site'],
  可比对的作品: ['可比對的作品', '比較可能な作品', 'Comparable shows'],
  '私藏神作(我 ≥8,高出站均 ≥1.5)': ['私藏神作(我 ≥8,高出站均 ≥1.5)', '隠れた神作(自分≥8、平均より+1.5以上)', 'Hidden gems (me ≥8, ≥1.5 above site)'],
  '众人皆醉(我 ≤5,低于站均 ≥2)': ['眾人皆醉(我 ≤5,低於站均 ≥2)', '世間とズレた作品(自分≤5、平均より−2以下)', 'Against the crowd (me ≤5, ≥2 below site)'],
  弃番解剖: ['棄番解剖', '切り解剖', 'Drop anatomy'],
  '一部都没弃过,了不起': ['一部都沒棄過,了不起', '1本も切っていない、すごい', 'Never dropped a show. Impressive'],
  弃番数: ['棄番數', '切った本数', 'Dropped'],
  平均弃在: ['平均棄在', '平均で切る話数', 'Avg drop point'],
  '第 {n} 话': ['第 {n} 話', '第 {n} 話', 'ep {n}'],
  '{len} 话完走率': ['{len} 話完走率', '{len}話の完走率', 'Finish rate ({len} eps)'],
  弃在第几话: ['棄在第幾話', '何話で切ったか', 'Dropped at episode'],
  '最容易弃的题材(≥4 部)': ['最容易棄的題材(≥4 部)', '切りやすいジャンル(4作以上)', 'Most-dropped genres (≥4 shows)'],
  '追新 vs 补番': ['追新 vs 補番', '新作追い vs 積み消化', 'New vs backlog'],
  追新率: ['追新率', '新作追い率', 'Same-season rate'],
  '开播 120 天内看完': ['開播 120 天內看完', '放送開始から120日以内に完走', 'finished within 120 days of airing'],
  平均滞后: ['平均滯後', '平均遅れ', 'Avg lag'],
  开播到看完: ['開播到看完', '放送開始から完走まで', 'from airing to finish'],
  '每年看完的番里,补的往年番有多少': ['每年看完的番裡,補的往年番有多少', '各年に完走した作品のうち過去作の割合', 'Of shows finished each year, how many were backlog'],
  '看完 {n} 部,其中补番 {m} 部': ['看完 {n} 部,其中補番 {m} 部', '完走 {n} 本、うち過去作 {m} 本', '{n} finished, {m} from backlog'],
  当年新番: ['當年新番', 'その年の新作', 'Same-year shows'],
  补往年的番: ['補往年的番', '過去作', 'Backlog'],
  冷门指数: ['冷門指數', 'マイナー度', 'Obscurity index'],
  看过作品的站内收藏人数中位数: ['看過作品的站內收藏人數中位數', '視聴作品の収蔵者数の中央値', 'median collectors of shows you watched'],
  你看过的最冷门的几部: ['你看過的最冷門的幾部', '視聴した中で最もマイナーな作品', 'The most obscure shows you watched'],
  '{n} 人': ['{n} 人', '{n} 人', '{n} people'],
  口味演变: ['口味演變', '嗜好の変化', 'Taste over time'],
  '按首播年,看过/在看作品的题材占比': ['按首播年,看過/在看作品的題材佔比', '放送年ごとのジャンル比率(視聴/視聴中)', 'genre share of watched shows by airing year'],
  其他: ['其他', 'その他', 'Other'],
  '制作公司 / 导演 / 声优榜': ['製作公司 / 導演 / 聲優榜', '制作会社 / 監督 / 声優ランキング', 'Studios / directors / voice actors'],
  '要拉取 {n} 部条目的制作与角色信息(官方 API,每部两次请求,结果缓存 30 天)': ['要拉取 {n} 部條目的製作與角色資訊(官方 API,每部兩次請求,結果快取 30 天)', '{n} 作品の制作・キャスト情報を取得します(公式API、1作品2リクエスト、30日キャッシュ)', 'Fetch staff & cast for {n} shows (official API, 2 requests each, cached 30 days)'],
  开始统计: ['開始統計', '集計開始', 'Compute'],
  '已拉取 {a} / {b}': ['已拉取 {a} / {b}', '取得済 {a} / {b}', 'Fetched {a} / {b}'],
  制作公司: ['製作公司', '制作会社', 'Studio'],
  导演: ['導演', '監督', 'Director'],
  '声优(主角/配角)': ['聲優(主角/配角)', '声優(主役/脇役)', 'Voice actors (main/supporting)'],
  '最右一列是我给这些作品的平均分——验证"某声优出场即高分"的迷信': ['最右一列是我給這些作品的平均分——驗證「某聲優出場即高分」的迷信', '右端は自分がつけた平均点。「この声優が出れば高評価」説の検証用', 'Right column = my average score for their shows — test the "this VA means a 9" superstition'],
  好友重合度: ['好友重合度', 'フレンドとの重なり', 'Friend overlap'],
  '在「设置 → 好友」里填上 bgm 用户名,这里就能算口味相似度': ['在「設定 → 好友」裡填上 bgm 使用者名稱,這裡就能算口味相似度', '「設定 → フレンド」にbgmのユーザー名を入れると嗜好の類似度を計算できます', 'Add bgm usernames under Settings → Friends to compare taste'],
  共同收藏: ['共同收藏', '共通の収蔵', 'Both collected'],
  共同看过: ['共同看過', '共に視聴', 'Both watched'],
  口味相似度: ['口味相似度', '嗜好の類似度', 'Taste match'],
  '共同打分 {n} 部': ['共同打分 {n} 部', '共通で評価 {n} 作品', '{n} rated by both'],
  样本不足: ['樣本不足', 'サンプル不足', 'too few'],
  计算: ['計算', '計算', 'Compute'],
  ...
  搁置: ['擱置', '保留', 'On hold'],
  '共 {n} 部': ['共 {n} 部', '全 {n} 作品', '{n} shows'],
  '更新于 {t}': ['更新於 {t}', '{t} 更新', 'updated {t}'],
  刷新: ['重新整理', '更新', 'Refresh'],
  '正在拉取全部收藏…': ['正在拉取全部收藏…', '全収蔵を取得中…', 'Fetching your full collection…'],
  '登录 bgm 账号后可统计全部历史收藏;现在只统计本机记录的当季追番。': [
    '登入 bgm 帳號後可統計全部歷史收藏;現在只統計本機記錄的當季追番。',
    'bgmにログインすると全履歴を集計できます。今はこの端末に記録された今期分のみです。',
    'Sign in to bgm to include your whole history; showing only this device’s current-season records for now.',
  ],
  '约 {n} 集': ['約 {n} 集', '約 {n} 話', '~{n} eps'],
  追番年数: ['追番年數', '視聴歴', 'Years'],
  '{n} 年': ['{n} 年', '{n} 年', '{n} yrs'],
  '自 {y} 年起': ['自 {y} 年起', '{y} 年から', 'since {y}'],
  平均打分: ['平均打分', '平均評価', 'My avg'],
  '站均 {s}': ['站均 {s}', 'サイト平均 {s}', 'site avg {s}'],
  每季追番: ['每季追番', 'シーズン別', 'Per season'],
  每年追番: ['每年追番', '年別', 'Per year'],
  '点击柱子跳到该季归档(2014 年起)': ['點擊柱子跳到該季歸檔(2014 年起)', 'クリックでそのシーズンのアーカイブへ(2014年〜)', 'Click a bar to open that season’s archive (2014+)'],
  换季对比: ['換季對比', 'シーズン比較', 'Season vs season'],
  本季: ['本季', '今期', 'This season'],
  上季: ['上季', '前期', 'Last season'],
  完走率: ['完走率', '完走率', 'Finish rate'],
  '完走率 = 看过 ÷ (看过 + 抛弃)': ['完走率 = 看過 ÷ (看過 + 拋棄)', '完走率 = 見た ÷ (見た + 切った)', 'Finish rate = done ÷ (done + dropped)'],
  评分分布: ['評分分布', '評価分布', 'Rating spread'],
  '打过分 {n} 部': ['打過分 {n} 部', '評価済 {n} 作品', '{n} rated'],
  题材偏好: ['題材偏好', 'ジャンル傾向', 'Genre taste'],
  年度足迹: ['年度足跡', '年別の記録', 'By year'],
  年份: ['年份', '年', 'Year'],
  均分: ['均分', '平均', 'Avg'],
  最高分: ['最高分', '最高評価', 'Top pick'],
  '我的神作(9 分以上)': ['我的神作(9 分以上)', '神作(9点以上)', 'My 9+ picks'],
  暂无数据: ['暫無資料', 'データなし', 'No data'],
  '拉取失败:{e}': ['拉取失敗:{e}', '取得失敗:{e}', 'Fetch failed: {e}'],
  '数据来自你的 Bangumi 收藏': ['資料來自你的 Bangumi 收藏', 'あなたのBangumi収蔵から集計', 'From your Bangumi collection'],
  '收藏太多,超出 4000 部的部分未计入': ['收藏太多,超出 4000 部的部分未計入', '収蔵が多すぎるため 4000 作品以降は未集計', 'Library too large: entries beyond 4000 were skipped'],
  '{n} 部': ['{n} 部', '{n} 作品', '{n}'],
  // ── 顶栏 / 工具栏 ──
  ...
  '🔭 前瞻': ['🔭 前瞻', '🔭 次期', '🔭 Preview'],
  '距开播 {n} 天': ['距開播 {n} 天', '開始まで {n} 日', '{n} days to go'],
  网络放送: ['網路放送', '配信', 'Streaming'],
  日期待定: ['日期待定', '日程未定', 'Date TBA'],
  首播: ['首播', '初回', 'Premiere'],
  '(下季)': ['(下季)', '(次期)', ' (next)'],
  下季新番: ['下季新番', '次期の新作', 'Next season'],
  时刻未定: ['時刻未定', '時刻未定', 'Time TBA'],
  '放送时刻待 bangumi-data 收录后自动补上': ['放送時刻待 bangumi-data 收錄後自動補上', '放送時刻は bangumi-data 収録後に自動反映', 'Air time fills in automatically once bangumi-data has it'],
  '{n} 人想看': ['{n} 人想看', '{n} 人が見たい', '{n} want to watch'],
  'bgm 想看人数': ['bgm 想看人數', 'bgm の「見たい」数', 'bgm wish count'],
  深夜: ['深夜', '深夜', 'Late night'],
  'B站番剧': ['B站番劇', 'bilibili アニメ', 'bilibili anime'],
  'B站搜索': ['B站搜尋', 'bilibili 検索', 'bilibili search'],
  '在 B 站番剧区搜索(正版引进)': ['在 B 站番劇區搜尋(正版引進)', 'bilibili のアニメ枠で検索(正規配信)', 'Search bilibili anime section (licensed)'],
  '在 B 站全站搜索': ['在 B 站全站搜尋', 'bilibili 全体を検索', 'Search all of bilibili'],
  '🗣 讨论会': ['🗣 討論會', '🗣 座談会', '🗣 Discuss'],
  '{s} 新番讨论会': ['{s} 新番討論會', '{s} 新作座談会', '{s} discussion'],
  '已过 {n} 部': ['已過 {n} 部', '{n} 作品済み', '{n} reviewed'],
  '想看 {a} · 观望 {b} · 跳过 {c}': ['想看 {a} · 觀望 {b} · 跳過 {c}', '見たい {a} · 保留 {b} · スキップ {c}', 'wish {a} · maybe {b} · skip {c}'],
  观望: ['觀望', '保留', 'Maybe'],
  跳过: ['跳過', 'スキップ', 'Skip'],
  上一部: ['上一部', '前へ', 'Previous'],
  下一部: ['下一部', '次へ', 'Next'],
  按首播日: ['按首播日', '放送日順', 'By premiere'],
  按期待: ['按期待', '期待順', 'By wish'],
  系列构成: ['系列構成', 'シリーズ構成', 'Series composition'],
  人物设定: ['人物設定', 'キャラクターデザイン', 'Character design'],
  声优: ['聲優', '声優', 'Cast'],
  '期待 {n}': ['期待 {n}', '期待 {n}', '{n} wish'],
  我: ['我', '自分', 'Me'],
  大家: ['大家', 'みんな', 'Everyone'],
  还没标: ['還沒標', '未登録', 'Not yet'],
  暂无简介: ['暫無簡介', 'あらすじなし', 'No synopsis'],
  '制作/声优载入中…': ['製作/聲優載入中…', '制作・声優 読み込み中…', 'Loading staff & cast…'],
  暂无下季数据: ['暫無下季資料', '次期データなし', 'No next-season data yet'],
  '点行跳转,← → 翻页;点 = 每个人的态度(我 · 好友)': ['點行跳轉,← → 翻頁;點 = 每個人的態度(我 · 好友)', '行をクリックで移動、← → でページ送り。点 = 各人の態度(自分・フレンド)', 'Click a row to jump, ← → to page; dots = each person (me · friends)'],
  '在「设置 → 好友」里填上 bgm 用户名,这里会显示他们的想看': ['在「設定 → 好友」裡填上 bgm 使用者名,這裡會顯示他們的想看', '「設定 → フレンド」に bgm ユーザー名を入れると「見たい」がここに出ます', 'Add friends\' bgm usernames in Settings → Friends to see their wishes here'],
  '名单来自 yuc.wiki 新番表,封面/期待/简介/制作/声优来自 bgm.tv;好友的想看来自其 bgm 公开收藏,每天更新一次。想看会同步到你的 bgm 收藏,观望/跳过只存本机。': ['名單來自 yuc.wiki 新番表,封面/期待/簡介/製作/聲優來自 bgm.tv;好友的想看來自其 bgm 公開收藏,每天更新一次。想看會同步到你的 bgm 收藏,觀望/跳過只存本機。', '作品リストは yuc.wiki の新作表、表紙・期待数・あらすじ・制作・声優は bgm.tv より。フレンドの「見たい」は bgm の公開収蔵から日次更新。「見たい」は bgm に同期、保留/スキップは端末内のみ。', 'Lineup from the yuc.wiki season table; covers, wish counts, synopsis, staff and cast from bgm.tv. Friends\' wishes come from their public bgm collections, refreshed daily. Wish syncs to your bgm collection; maybe/skip stay on this device.'],
  开房间: ['開房間', 'ルームを作る', 'Open a room'],
  关闭房间: ['關閉房間', 'ルームを閉じる', 'Close room'],
  复制链接: ['複製連結', 'リンクをコピー', 'Copy link'],
  已复制: ['已複製', 'コピーしました', 'Copied'],
  '{n} 人在线': ['{n} 人在線', '{n} 人参加中', '{n} online'],
  '房间 {code}': ['房間 {code}', 'ルーム {code}', 'Room {code}'],
  房间已过期或已关闭: ['房間已過期或已關閉', 'ルームは期限切れか閉じられました', 'Room expired or closed'],
  '开房间失败:{e}': ['開房間失敗:{e}', 'ルーム作成に失敗:{e}', 'Could not open a room: {e}'],
  '关闭房间?参会者将无法再投票。': ['關閉房間?參會者將無法再投票。', 'ルームを閉じますか?参加者は投票できなくなります。', 'Close the room? Participants will no longer be able to vote.'],
  '镜像站没有房间服务,请用 bgmtimetable.com 打开': ['鏡像站沒有房間服務,請用 bgmtimetable.com 打開', 'ミラーサイトにはルーム機能がありません。bgmtimetable.com で開いてください', 'The mirror has no room service — open bgmtimetable.com instead'],
  复制这个链接发给大家: ['複製這個連結發給大家', 'このリンクをみんなに送ってください', 'Copy this link and share it'],
  '开一个房间,大家用手机链接投票': ['開一個房間,大家用手機連結投票', 'ルームを作ると、みんながスマホのリンクから投票できます', 'Open a room so everyone can vote from a phone link'],
  'bgm 想看': ['bgm 想看', 'bgm 見たい', 'bgm wish'],
  主持人: ['主持人', '司会', 'Host'],
  你的昵称: ['你的暱稱', 'ニックネーム', 'Your nickname'],
  大家认得出你的名字: ['大家認得出你的名字', 'みんなにわかる名前で', 'A name everyone recognizes'],
  进入房间: ['進入房間', '入室', 'Join room'],
  '不用登录,昵称只存在你的手机上;主持人翻页时这里会自动跟着切。': ['不用登入,暱稱只存在你的手機上;主持人翻頁時這裡會自動跟著切。', 'ログイン不要。ニックネームは端末内のみ。司会がページを送ると自動で切り替わります。', 'No sign-in needed; your nickname stays on this device. The page follows the host automatically.'],
  房间不存在或已过期: ['房間不存在或已過期', 'ルームが存在しないか期限切れです', 'Room not found or expired'],
  房间人数已满: ['房間人數已滿', 'ルームが満員です', 'Room is full'],
  房间已结束: ['房間已結束', 'ルームは終了しました', 'The room has ended'],
  出错了: ['出錯了', 'エラー', 'Something went wrong'],
  '等待主持人开始…': ['等待主持人開始…', '司会の開始を待っています…', 'Waiting for the host…'],
  '第 {i} / {n} 部': ['第 {i} / {n} 部', '{i} / {n} 作品目', '{i} / {n}'],
  扫码进入房间: ['掃碼進入房間', 'QR コードで入室', 'Scan to join'],
  '扫码或打开链接,填昵称即可投票': ['掃碼或打開連結,填暱稱即可投票', 'QR コードを読むかリンクを開き、ニックネームを入れて投票', 'Scan or open the link, enter a nickname and vote'],
  共识榜: ['共識榜', 'みんなの結論', 'Consensus'],
  '{s} 新番共识榜': ['{s} 新番共識榜', '{s} 新作みんなの結論', '{s} consensus board'],
  本季共识榜: ['本季共識榜', '今期みんなの結論', 'Season consensus'],
  '按大家的票分层:都想看 / 有人想看 / 观望 / 跳过': ['按大家的票分層:都想看 / 有人想看 / 觀望 / 跳過', 'みんなの票で分類:全員見たい / 誰か見たい / 保留 / スキップ', 'Grouped by votes: everyone / someone / maybe / skip'],
  都想看: ['都想看', '全員見たい', 'Everyone wants'],
  有人想看: ['有人想看', '誰かが見たい', 'Someone wants'],
  '未过 {n} 部': ['未過 {n} 部', '未確認 {n} 作品', '{n} not reviewed'],
  '{n} 人参与:{names}': ['{n} 人參與:{names}', '{n} 人参加:{names}', '{n} people: {names}'],
  复制文本: ['複製文字', 'テキストをコピー', 'Copy text'],
  复制下面的文字: ['複製下面的文字', '下のテキストをコピー', 'Copy the text below'],
  导出图片: ['匯出圖片', '画像で保存', 'Export image'],
  '导出中…': ['匯出中…', '書き出し中…', 'Exporting…'],
  还没有人表态: ['還沒有人表態', 'まだ誰も投票していません', 'No votes yet'],
