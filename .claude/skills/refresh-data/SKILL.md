...
> 每集精确时刻的**自动层**已由 `.github/workflows/refresh-data.yml` 承担:
> 每日经 bgm.wiki 开发者 API(`BGM_WIKI_TOKEN` secret)同步 `air.epDates` 并开 PR。
> 本 skill 只处理它覆盖不到的部分:yuc 的标签/PV/改编来源,以及 bgm.wiki
> 未收录的流媒体先行(如整批提前配信)的人工判读。
